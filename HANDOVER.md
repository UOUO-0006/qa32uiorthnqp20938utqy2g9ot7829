...
2. **意味のあるクラス名・ID名・変数名・ファイル名の完全禁止**
   - ページ状態や機能を推測できる単語（`rotate` 等）を含めることは絶対禁止。
   - クラス・ID・変数名: `c`, `i`, `l`, `p` などの無機質・短縮記号のみ使用。
   - **ファイル名ルール**: **全てのファイル名は「完全な8桁のランダム英数字（例: `2X4EFtnr.html`, `RmT8tL0t.html`）」に統一すること（状態や意味を持たせない）。**
...
1. **ファイル構成**:
   - **全ページファイル名**: 完全な8桁のランダム英数字（例: `2X4EFtnr.html`, `RmT8tL0t.html`）
2. **更新運用**:
   - ページ内容を更新・改修するたびに、新しく生成した8桁ランダム英数字ファイルを作成し、旧ファイルは削除する。
3. **リンク難読化**:
...
> **【指示文】**  
> リポジトリ内の `HANDOVER.md` に記載されているARGWeb開発仕様（単語や状態を含まない**完全8桁ランダム英数字ファイル名**の徹底、親切設計の完全禁止、点字空白タイトル `U+2800` の維持、Base64難読化、`x12y16pxMaruMonica.ttf` の全編Webフォント適用、配置標準）を厳守してページの作成・更新を行ってください。
